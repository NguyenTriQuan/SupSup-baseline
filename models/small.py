...
class FC400(nn.Module):
    def __init__(self):
        super(FC400, self).__init__()
        builder = Builder()
        self.linear = nn.Sequential(
            builder.conv1x1(28 * 28, int(args.width_mult * 400), first_layer=True),
            nn.ReLU(),
            nn.Dropout(0.5),
            builder.conv1x1(int(args.width_mult * 400), int(args.width_mult * 400)),
            nn.ReLU(),
            nn.Dropout(0.5),
            builder.conv1x1(int(args.width_mult * 400), args.output_size, last_layer=True),
        )
