...
def train(
    model, writer, train_loader, train_transform, optimizer, criterion, epoch, task_idx, data_loader=None
):
    model.zero_grad()
    model.train()
...
def test(model, writer, criterion, test_loader, test_transform, epoch, task_idx):
    model.zero_grad()
    model.eval()
    test_loss = 0
    correct = 0
...
def gradient_minimization(
    adaptation_criterion, model, writer, criterion, test_loader, test_transform, adapt_lr, **kwargs
):
    model.zero_grad()
    model.train()
