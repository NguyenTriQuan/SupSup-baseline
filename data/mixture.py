...
class Mixture:
    def __init__(self):
        ...
        data,taskcla,size,idata = get(seed=args.seed,fixed_order=False,pc_valid=0.15,tasknum=8)
        # args.output_size = taskcla
        ...
        self.loaders = [
            (
                ...
                torch.utils.data.DataLoader(
                    data[t]['valid']['dataset'], batch_size=args.test_batch_size, shuffle=True
                ),
                torch.utils.data.DataLoader(
...
def get(seed=0,fixed_order=False,pc_valid=0.15,tasknum=8):
    ...
    if not os.path.isdir(args.data+'/binary_mixture/'):
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                dat={}
                dat['train']=datasets.CIFAR10(args.data,train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.CIFAR10(args.data,train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='cifar10'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                std=[x/255 for x in [63.0,62.1,66.7]]
                dat={}
                dat['train']=datasets.CIFAR100(args.data,train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.CIFAR100(args.data,train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                ...
                std=(0.2752,)
                dat={}
                dat['train']=datasets.MNIST(args.data,train=True,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.MNIST(args.data,train=False,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='mnist'
                data[n]['ncla']=10
                ...
                dat = {}
                dat['train']=datasets.SVHN(args.data,split='train',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.SVHN(args.data,split='test',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n] = {}
                data[n]['name']='svhn'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                std=(0.3318,)
                dat={}
                dat['train']=FashionMNIST(args.data+'/fashion_mnist', train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                dat['test']=FashionMNIST(args.data+'/fashion_mnist', train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                data[n]={}
                ...
                dat={}
                dat['train']=TrafficSigns(args.data+'/traffic_signs', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=TrafficSigns(args.data+'/traffic_signs', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                ...
                dat={}
                dat['train']=Facescrub(args.data+'/facescrub', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=Facescrub(args.data+'/facescrub', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                ...
                std=(0.4501,)
                dat={}
                dat['train']=notMNIST(args.data+'/notmnist', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=notMNIST(args.data+'/notmnist', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                #mean, std = utils.compute_mean_std_dataset(dat['train']); print(mean,std); sys.exit()
                data[n]={}
            ...
            for s in ['train','test']:
                ...
                torch.save(data[n][s]['x'], os.path.join(os.path.expanduser(args.data+'/binary_mixture'),'data'+str(idx)+s+'x.bin'))
                torch.save(data[n][s]['y'], os.path.join(os.path.expanduser(args.data+'/binary_mixture'),'data'+str(idx)+s+'y.bin'))
        ...
        for n,idx in enumerate(idata):
            ...
            for s in ['train','test']:
                data[n][s]={'x':[],'y':[]}
                data[n][s]['x'] = torch.load(os.path.join(os.path.expanduser(args.data+'/binary_mixture'),'data'+str(idx)+s+'x.bin'))
                data[n][s]['y'] = torch.load(os.path.join(os.path.expanduser(args.data+'/binary_mixture'),'data'+str(idx)+s+'y.bin'))
    ...
    for t in data.keys():
        ...
        data[t]['valid']['dataset'] = data_utils.TensorDataset(data[t]['valid']['x'], data[t]['valid']['y'])
